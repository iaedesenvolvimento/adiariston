...
import type { CreateVisitorResult } from "@/types/visitors";
...
export async function POST(request: NextRequest) {
  ...
  try {
    await createVisitor(validation.data);
      ...
      return json(
        {
          ok: false,
          message:
            "O cadastro ainda não está disponível porque o Supabase não foi configurado.",
        },
        { status: 503 }
      );
    }

    console.error("Erro ao cadastrar visitante.", {
      name:
